const { sha256Bytes, sha256 } = require('./sha256');
...
const hmacSha256 = (key, message) => {
  ...
  const inner = sha256Bytes(concatenate(xorBytes(keyBytes, ipad), messageBytes));
  return sha256(concatenate(xorBytes(keyBytes, opad), inner));
};
